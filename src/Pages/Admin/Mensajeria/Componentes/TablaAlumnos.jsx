...
export default function TablaAlumnos({
  ...
  const renderCelda = (alumno, columna) => {
    ...
    } else if (columna.key === "grado") {
      // Mostrar el gradoSeccion si existe, de lo contrario mostrar el valor de grado
      return (
        <Chip
          label={alumno.gradoSeccion || `Grado ${alumno.grado}`}
          color="primary"
          variant="outlined"
          className="grado-chip"
        />
      );
    } else if (columna.key === "sede") {
      // Mostrar el sedeNombre si existe, de lo contrario mostrar el valor de sede
      return (
        <Chip
          label={alumno.sedeNombre || `Sede ${alumno.sede}`}
          color="secondary"
          variant="outlined"
          className="sede-chip"
        />
          ...
          <TableBody>
            {alumnos.length === 0 ? (
              ...
              alumnos.map((alumno, index) => (
                ...
                >
                  {COLUMNAS_TABLA.map((columna) => (
                    <TableCell key={`${alumno.id}-${columna.key}`}>
                      {renderCelda(alumno, columna)}
                    </TableCell>
                  ))}
                </TableRow>
              ))
